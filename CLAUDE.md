...
- **`pyccode.permissions`** — Permission layer. `validate_command(command)` checks against `READONLY_ALLOWLIST` (ls/cat/pwd/grep/git status/git log in Task 1) using a flag-walking parser. `check_permission(tool, input)` returns `'allow'` or `'confirm'`. `prompt_user(action)` reads y/N. Handlers call this before mutating; on denial, return `"Error: Permission denied by user"`. See [permissions spec](.trellis/spec/backend/permissions.md).
- **`pyccode.hooks`** — Hook framework. `HookType.POST_TOOL_USE` fires external subprocess scripts after every tool call. Payload (12 fields) goes to hook's stdin as JSON; exit code 0 = success, non-zero = logged to stderr + chat loop continues. Configured via `~/.pyccode/settings.json`. MVP is read-only observer (no modify, no veto). See [hooks spec](.trellis/spec/backend/hooks.md).
...
- **`appendTranscript`** — Write-side transcript logger. Appends one JSON object per line to `~/.pyccode/projects/<sanitized-cwd>/<SESSION_ID>.jsonl` (sanitized cwd collapses non-`[A-Za-z0-9._-]` to `-`). Schema: `type` / `uuid` / `parentUuid` (chain) / `timestamp` / `sessionId` / `cwd` / `gitBranch` / `version` / `message` (assistant entries embed `message.usage`). Open-write-close per entry; `ensure_ascii=False` keeps non-ASCII verbatim. Whole body wrapped in try/except; failures print `[Transcript write failed: ...]` to **stderr** and never break the chat loop. Write-only in MVP — no resume logic. Writing is routed through the internal `MessageAppend` hook (see pyccode.hooks).
- **`history_append`** — Helper called by all `chat()` history mutations: does `history.append(...)` then fires the internal `MessageAppend` hook (transcript writes there). Assistant turns pass `usage` from `response.usage`. Covers: initial user prompt, assistant turn, tool-result user message, max-tokens continuation, TodoWrite round-counter reminder. `handle_subagent`'s `messages.append` is intentionally NOT routed here, so subagent turns stay un-transcribed in MVP.
...
- Permissions: default-confirm at both tool and bash levels. `ALLOWED_TOOLS = {read, TodoWrite, skill, run_subagent}` auto-pass; everything else (`bash`/`write`/`edit` and any future mutating tool) prompts the user (y/N). A built-in read-only allowlist (`ls`, `cat`, `pwd`, `grep`, `git status`, `git log` in Task 1) runs without prompting via flag-walking validation. On denial, handlers return `"Error: Permission denied by user"`. A future `DENY_LIST` tier (hard-deny, no prompt) is reserved. Defense-in-depth against accidents, not adversarial input — sandbox for the latter.
